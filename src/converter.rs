...
pub fn get_vectorized_image(p: PathBuf) -> String {
    ...
    let pixels = img.to_rgba8().as_raw().to_vec();
...
fn get_svg_string(paths: Vec<SvgPath>, dimension: Dimension) -> String {
    ...
    for p in paths.iter() {
        let (d, offset) = p.path.to_svg_string(true, PointF64::default(), Some(8));
...
mod tests {
    ...
    fn test_get_svg_string() {
        ...
        let svg = Tree::from_str(&svg_string.as_str(), &Options::default().to_ref());
